...
class TestComponent extends Component {
	...
	handleTestUpdateProfile = async () => {
		...
		// doc = diana's userUid
		let userDocRef = await firestore.collection('users').doc('NClRLP81qWhTPNcG53HK4E3qwaX2');
		try {
	...
	handleTestJoinEvent = async () => {
		...
		try {
			await eventDocRef.update({
				[`attendees.NClRLP81qWhTPNcG53HK4E3qwaX2`]: attendee,
			});
			toastr.success('Success');
		} catch (error) {
			console.log(error);
	...
	handleTestCancelGoingToEvent = async () => {
		...
		try {
			await eventDocRef.update({
				[`attendees.NClRLP81qWhTPNcG53HK4E3qwaX2`]: firebase.firestore.FieldValue.delete(),
			});
			toastr.success('Success');
		} catch (error) {
			console.log(error);
	...
	handleTestChangeAttendeePhotoInEvent = async () => {
		...
		try {
			await eventDocRef.update({
				[`attendees.NClRLP81qWhTPNcG53HK4E3qwaX2.photoURL`]: 'testing123.jpg',
			});
			toastr.success('Success');
		} catch (error) {
			console.log(error);
